...
const SyncEmpresas = (props: Props) => {
  ...
  const totalPlantasReducer = (total: number, empresa: Empresa) =>
    total + empresa.plantas.length;
    ...
    <Stack padding={7} flex={1} justifyContent='space-between'>
      <Stack space={5}>
        <Box borderColor='transparent' borderWidth='1' shadow={1} padding={4}>
          <Text bold color={brandColors.primary} mb={2}>
            Empresas
          </Text>
          <Text>
        ...
        <Box borderColor='coolGray.300' borderWidth='1' shadow={1} padding={4}>
          ...
          <Text>{totalPlantas} plantas armazenadas no total</Text>
        </Box>
        ...
        <Box borderColor='coolGray.300' borderWidth='1' shadow={1} padding={4}>
          ...
          <Text>
            {totalUsuarios} usuários armazenados e autorizados a utilizarem este
            dispostivo
          </Text>
        </Box>
      </Stack>
      <ActionButton isLoading={loading} onPress={() => atualizarEmpresas()}>
